...
class TemporalLightningModule(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        out = compute_train_batch_loss(
            ...
            teacher_z_small=self.teacher_z_small,
            device=self.device,
        )

        if out is None:
            loss = self.node_emb.weight.sum() * 0.0
            self.log("train_loss", 0.0, on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
            return loss
        ...
        self.log("train_loss_big", out["loss_big"], on_step=False, on_epoch=True, batch_size=out["n_users"], sync_dist=True)
        self.log("train_loss_small", out["loss_small"], on_step=False, on_epoch=True, batch_size=out["n_users"], sync_dist=True)
        self.log("train_distill_loss", out["distill_loss"], on_step=False, on_epoch=True, batch_size=out["n_users"], sync_dist=True)

        return out["loss"]

    def _load_teacher_embeddings(self, cfg):
        cache_path = getattr(cfg, "teacher_embeddings_path", "")
        if not cache_path:
            raise ValueError("teacher_embeddings_path must be set for offline_kd mode")
        teacher_cache = torch.load(cache_path, map_location="cpu")
        if "z_small" not in teacher_cache:
            raise ValueError("teacher_embeddings_path must contain 'z_small'")
        self.teacher_z_small = teacher_cache["z_small"].float()
    ...
    def on_validation_epoch_end(self):
        ...
        self.log("val_ndcg_small", metrics["ndcg_small"], on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
        self.log("val_coverage_big", metrics["coverage_big"], on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
        self.log("val_coverage_small", metrics["coverage_small"], on_step=False, on_epoch=True, prog_bar=False, sync_dist=True)
