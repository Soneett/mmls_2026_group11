...
В проект добавлены два режима для ускорения обучения:

- **DDP**: параллелизация между устройствами.
- **Gradient accumulation**: накопление градиента на нескольких микробатчах.

Важно: обучение в проекте идет по временным срезам `sid`, поэтому распараллеливание сделано **внутри текущего `sid` по пользователям**, а не по самим `sid`.

Ключевые параметры в `configs/base.yaml`:

- `parallel_mode`: `"none"` или `"ddp"`
- `devices`: число устройств для Lightning Trainer
- `grad_accum_steps`: число шагов накопления градиента
- `users_per_batch`: размер микробатча по пользователям внутри одного `sid` (`0` = все пользователи среза)
